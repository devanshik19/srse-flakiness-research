...
**90 rows** -- 34 fully checked (ND/ID/OD, all clean: 0 flaky), 0 INCONCLUSIVE_OVERWRITE (file-overwrite artifact, not real flakiness -- see notes), 14 CANDIDATE_FAILED, 4 GEN_FAILED, 38 SKIPPED_FOCAL

Java-WebSocket is **complete: 90/90 non-flaky tests processed**, including the 3 tests that hit the file-overwrite bug (blocker #7) -- all 3 have been regenerated and re-checked after fixing that bug plus a second build-breaking bug (blocker #8, hardcoded compiler source 1.6). 2 came back clean, 1 is a genuine CANDIDATE_FAILED (subject: `TooTallNate/Java-WebSocket`, commit `fa3909c391195178ccf5a92d4ac342a30ae247c8`, module `.`).
...
| util.ByteBufferUtilsTest#testTransferByteBufferCheckNullDest | util.ByteBufferUtils#transferByteBuffer | GENERATED (candidate failed) | NA | NA | NA | CANDIDATE_FAILED (did not pass alone): Tests run: 6, Failures: 2 -- regenerated after fixing blockers #7/#8; this attempt's candidate genuinely failed. The ORIGINAL batch3 candidate for this source test was lost to the file-overwrite bug before it could ever be checked, so its true status was never confirmed -- this regeneration is the first real verdict for this test. |
